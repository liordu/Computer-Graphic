...
class DepthBuffer {
    ...
    TestAndSetFragment(x, valueF, depthTestMode) {
        ...
        if (depthTestMode == 1 && oldValue > newValue) {
...
class RenderingPipeline {
    ...
    PrimitiveAssemblyStage(vertexStream, ibo) {
        ...
        for(let i = 0; i < ibo.length / 2; i++){
            primitives[i] = new Array(2);
            primitives[i][0] = vertexStream[ibo[2*i]]; 
            primitives[i][1] = vertexStream[ibo[2*i+1]];
        }
    ...
    LineCulling(a, b) {
        ...
        let dehom_a = [a[0]/a[2], a[1]/a[2]];
        let dehom_b = [b[0]/b[2], b[1]/b[2]];
        //console.log("a: " + dehom_a[0]);
        if(dehom_a[0] < dehom_b[0]){
            // counterclockwise => front face
            return this.culling == -1;
        }else{
            // clockwise => back face
            return this.culling == 1;
        }
